...
class ViewController: UIViewController, WKNavigationDelegate {
    ...
    @objc func keyboardWillHide(_ notification: NSNotification) {
        AuthDemo.webView.setNeedsLayout()
    }

    func initWebView() {
        AuthDemo.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(AuthDemo.webView);
        AuthDemo.webView.uiDelegate = self;
        AuthDemo.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = AuthDemo.webView.observe(\.underPageBackgroundColor) { [unowned self] webView, _ in
                currentWebViewTheme = AuthDemo.webView.underPageBackgroundColor.isLight() ?? true ? .light : .dark
...
//        self.splashBkgView.frame = CGRect(x: splashBkgFrame.minX, y: statusBarHeight, width: splashBkgFrame.width, height: splashBkgFrame.height)
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !AuthDemo.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl() {
        ...
        AuthDemo.webView.load(URLRequest(url: launchUrl))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            AuthDemo.webView.isHidden = false;
    ...
    func webView(_ webView: WKWebView, didFailProvisionalNavigation navigation: WKNavigation!, withError error: Error) {
        ...
        if (error as NSError)._code != (-999) {
            print("Failed to load URL: \(self.lastURL)")
            sentryCaptureEvent(level: .error, message: "Failed to load external page on iOS", extra: ["prevURL": self.prevURL, "failedURL": self.lastURL])
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                AuthDemo.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(AuthDemo.webView.estimatedProgress);
    ...
    deinit {
        AuthDemo.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: AuthDemo.webView)
        }
        ...
        if message.name == "openLinkInBrowser" {
            guard let link = message.body as? String else {
                return
            }
            openLinkInBrowser(link: link)
        }
        if message.name == "socialAuth" {
            guard let provider = message.body as? String else {
                return
            }
            let socialAuth = SocialAuth();
            socialAuth.signIn(provider: provider);
        }
  }
}
