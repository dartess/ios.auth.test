...
func createWebView(container: UIView, WKSMH: WKScriptMessageHandler, WKND: WKNavigationDelegate, NSO: NSObject, VC: ViewController) -> WKWebView{
    ...
    userContentController.add(WKSMH, name: "openLinkInBrowser")
...
func setCustomCookie(webView: WKWebView) {
    let domain = rootUrl.host!
    let domainParts = domain.split(separator: ".");
    let parentDomain = domainParts.count >= 3
        ...
        if let requestUrl = navigationAction.request.url{
            if let requestHost = requestUrl.host {
                ...
                if (matchingHostOrigin != nil) {
                    ...
                    let matchingHostOriginForcedPopup = allowedOriginsForcedPopupMode.first(where: { requestHost.range(of: $0) != nil })
                    if (matchingHostOriginForcedPopup != nil) {
                        // open in popup
